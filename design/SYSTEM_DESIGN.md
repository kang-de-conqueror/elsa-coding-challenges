...
- **Vue 3 client** (`client/`): one WebSocket connection per browser tab. `useQuizSocket.ts` owns the connection lifecycle (connect, reconnect with exponential backoff, heartbeat ping, rejoin from a saved session). A Pinia store (`stores/quiz.ts`) holds all quiz state as the single source of truth for the UI; components (`QuestionCard`, `LeaderboardPanel`, `ScoreBadge`, `ConnectionStatus`) are pure renderers of that state.
...
5. **Disconnect/rejoin**: on a clean disconnect, the server marks the participant `connected: false` and broadcasts `participant_update`; the client's saved `{quiz_id, user_id}` (localStorage) lets `tryRejoin` restore the exact same participant (and score) on reconnect via `{type: "rejoin"}`.
...
| Client tooling | ESLint + oxlint, vue-tsc, Vitest (scaffolded) | Matches the create-vue "recommended" toolchain; consistent with the server's lint/type-check/test three-step |
...
- **Reliability**: idempotent answer submission (dedup by `request_id`), rejoin-with-preserved-score, idle-connection reaping, graceful per-connection error handling (a malformed message or a broken socket never crashes the room for other participants — see `connection_manager.py::_safe_send`).
...
- **Full frontend end-to-end test suite (Playwright)** — the client ships with Vitest scaffolding (from `create-vue`) but no component/e2e tests were added, given the time budget for this challenge; the WebSocket protocol itself is covered end-to-end by the server's integration test (`server/tests/test_integration_ws.py`), which exercises the exact same message contract the Vue client speaks.
